    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_1418.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _1_var = UnpackNormal(tex2D(_1,TRANSFORM_TEX(i.uv0, _1)));
                float4 node_1418 = _Time;
                float node_133 = pow((abs((frac((i.uv0+node_1418.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 normalLocal = normalize(lerp(_1_var.rgb,float3(0,0,1),node_133));
...
////// Specular:
                ...
                float3 specularColor = node_4921;
                float specularMonochrome;
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float node_8608 = 0.1;
                float3 diffuseColor = lerp(_Diffuse_var.rgb,float3(node_8608,node_8608,node_8608),node_133); // Need this for specular when using metallic
                ...
                float3 indirectSpecular = (0 + float3(node_4921,node_4921,node_4921));
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.tangentDir = normalize( mul( unity_ObjectToWorld, float4( v.tangent.xyz, 0.0 ) ).xyz );
                o.bitangentDir = normalize(cross(o.normalDir, o.tangentDir) * v.tangent.w);
                float4 node_841 = _Time;
                float node_133 = pow((abs((frac((o.uv0+node_841.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                float3 lightColor = _LightColor0.rgb;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _1_var = UnpackNormal(tex2D(_1,TRANSFORM_TEX(i.uv0, _1)));
                float4 node_841 = _Time;
                float node_133 = pow((abs((frac((i.uv0+node_841.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 normalLocal = normalize(lerp(_1_var.rgb,float3(0,0,1),node_133));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_8904 = _Time;
                float node_133 = pow((abs((frac((o.uv0+node_8904.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                o.pos = UnityObjectToClipPos( v.vertex );
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_1599 = _Time;
                float node_133 = pow((abs((frac((o.uv0+node_1599.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 node_1599 = _Time;
                float node_133 = pow((abs((frac((i.uv0+node_1599.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                ...
                float3 specColor;
                float node_4921 = 1.0;
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, node_4921, specColor, specularMonochrome );
