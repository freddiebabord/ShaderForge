...
Shader "Shader Forge/Examples/Custom Lighting" {
    Properties {
        ...
        _Gloss ("Gloss", Range(0, 1)) = 0.1848591
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float3 node_84 = (_Diffuse_var.rgb*_Color.rgb*floor(max(0,dot(lightDirection,normalDirection)) * _Bands) / (_Bands - 1)); // Diffuse Light
                float3 emissive = node_84;
                float3 finalColor = emissive + (node_84+UNITY_LIGHTMODEL_AMBIENT.rgb+floor(pow(max(0,dot(normalDirection,halfDirection)),exp2(((_Gloss*10.0)+1.0))) * _Bands) / (_Bands - 1));
...
////// Lighting:
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float3 node_84 = (_Diffuse_var.rgb*_Color.rgb*floor(max(0,dot(lightDirection,normalDirection)) * _Bands) / (_Bands - 1)); // Diffuse Light
                float3 finalColor = (node_84+UNITY_LIGHTMODEL_AMBIENT.rgb+floor(pow(max(0,dot(normalDirection,halfDirection)),exp2(((_Gloss*10.0)+1.0))) * _Bands) / (_Bands - 1));
